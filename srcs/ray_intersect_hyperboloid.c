/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ray_intersect_hyperboloid.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: arthur <arthur@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/06/18 10:02:42 by vle-guen          #+#    #+#             */
/*   Updated: 2016/06/27 22:28:35 by avella           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"
#include <math.h>

static int		rescue_texture_color(t_ray *r, t_obj *o)
{
	double	yaw;
	double	pitch;
	double	x;
	double	y;
	t_pix	p;

	pitch = (M_IJ(&(r->normal), 1, 0) > 0 ? 1 : -1)
		* acos(M_IJ(&(r->normal), 0, 0) / sqrt(M_IJ(&(r->normal), 0, 0)
		* M_IJ(&(r->normal), 0, 0) + M_IJ(&(r->normal), 1, 0)
		* M_IJ(&(r->normal), 1, 0)));
	yaw = -acos(M_IJ(&(r->normal), 2, 0));
	x = (pitch + 3.1415) / (2 * 3.1415);
	y = fabs((yaw - 3.1415) / 3.1415);
	x = x * M_IJ(&(o->param.text_mod), 0, 0) + M_IJ(&(o->param.text_mod), 0, 1);
	y = y * M_IJ(&(o->param.text_mod), 1, 0) + M_IJ(&(o->param.text_mod), 1, 1);
	ft_draw_getp(x, y, &p, o->param.texture);
	r->obj->param.color = p.c;
	return (0);
}

static int		ray_solution_hyperboloid(t_ray *ray, t_obj *obj, double k)
{
	int		sign;

	ray->obj = obj;
	ray->intersection = M_POINT(
		M_IJ(&(ray->start), 0, 0) + ray->dist * M_IJ(&(ray->dir), 0, 0),
		M_IJ(&(ray->start), 1, 0) + ray->dist * M_IJ(&(ray->dir), 1, 0),
		M_IJ(&(ray->start), 2, 0) + ray->dist * M_IJ(&(ray->dir), 2, 0));
	if (mat_norm(&(ray->start)) > 1)
		sign = 1;
	else
		sign = -1;
	ray->normal = M_DIR(
		sign * 2 * M_IJ(&(ray->intersection), 0, 0),
		sign * (-2) * M_IJ(&(ray->intersection), 1, 0) * k,
		sign * 2 * M_IJ(&(ray->intersection), 2, 0));
	if (obj->param.texture)
		rescue_texture_color(ray, obj);
	return (0);
}

int				ray_intersect_hyperboloid(t_ray *ray, t_obj *obj)
{
	double	a;
	double	b;
	double	c;
	double	k;

	k = (pow(obj->param.radius, 2) - 1) / pow(obj->param.height, 2);
	a = pow(M_IJ(&(ray->dir), 0, 0), 2) + pow(M_IJ(&(ray->dir), 2, 0), 2)
		- k * pow(M_IJ(&(ray->dir), 1, 0), 2);
	b = 2 * (M_IJ(&(ray->dir), 0, 0) * M_IJ(&(ray->start), 0, 0)
		+ M_IJ(&(ray->dir), 2, 0) * M_IJ(&(ray->start), 2, 0)
		- k * M_IJ(&(ray->dir), 1, 0) * M_IJ(&(ray->start), 1, 0));
	c = pow(M_IJ(&(ray->start), 0, 0), 2) + pow(M_IJ(&(ray->start), 2, 0), 2)
		- k * pow(M_IJ(&(ray->start), 1, 0), 2) - 1;
	ray->dist = solve_2_degree_neg(a, b, c, ray);
	if (ray->dist > 0 && (M_IJ(&(ray->start), 1, 0) + ray->dist
		* M_IJ(&(ray->dir), 1, 0) >= -1 * (obj->param.height))
		&& (M_IJ(&(ray->start), 1, 0) + ray->dist
		* M_IJ(&(ray->dir), 1, 0) <= obj->param.height))
		ray_solution_hyperboloid(ray, obj, k);
	return (0);
}
