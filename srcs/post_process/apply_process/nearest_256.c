...
/*   Updated: 2016/06/18 07:30:06 by igomez           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_find_nearest_256(uint8_t r, uint8_t g, uint8_t b)
{
	int dist;
	int new_dist;
	int i;
	...
	while (i < 255)
	{
		c[0] = N_ABS(r - (uint8_t)((g_data2[i] >> 16) & 0xff));
		c[1] = N_ABS(g - (uint8_t)((g_data2[i] >> 8) & 0xff));
		c[2] = N_ABS(b - (uint8_t)((g_data2[i]) & 0xff));
...
void		ft_apply_nearest_256(t_env *e, int x, int y)
{
	...
	new = ft_find_nearest_256(uc.r, uc.g, uc.b);
